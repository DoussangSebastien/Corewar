...
static int get_xor_value(int param, int i, process_t *process,
    corewar_t *corewar)
{
    int reg = 0;
...
static void get_xor_params(int *params, int *temp_pc, process_t *process,
    corewar_t *corewar)
{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        params[i] = get_xor_value(params_type[i], *temp_pc, process, corewar);
...
int xor_command(process_t *process, corewar_t *corewar)
{
    ...
    if (!process || !corewar)
        return FAILURE;
    temp_pc = process->pc + 2;
    get_xor_params(params, &temp_pc, process, corewar);
