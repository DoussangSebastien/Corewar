...
static int get_and_value(int param, int i, process_t *process,
    corewar_t *corewar)
{
    int reg = 0;
...
static void get_and_params(int *params, int *temp_pc, process_t *process,
    corewar_t *corewar)
{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        params[i] = get_and_value(params_type[i], *temp_pc, process, corewar);
...
int and_command(process_t *process, corewar_t *corewar)
{
    ...
    get_and_params(params, &temp_pc, process, corewar);
