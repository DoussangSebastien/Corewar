...
static int get_ldi_value(int param, int i, process_t *process,
    corewar_t *corewar)
{
    int reg = 0;
    ...
    if (param == T_REG) {
        ...
        return get_value_mod(param == T_DIR, i, process->pc, corewar->arena);
}

static void get_ldi_params(unsigned short *params, int *temp_pc,
    process_t *process, corewar_t *corewar)
{
    ...
    params[0] = get_ldi_value(params_type[0], *temp_pc, process, corewar);
...
int ldi(process_t *process, corewar_t *corewar)
{
    ...
    get_ldi_params(params, &temp_pc, process, corewar);
    s = params[0] + params[1];
    reg = corewar->arena[temp_pc] - 1;
    if (reg >= 0 && reg < REG_NUMBER)
        process->regs[reg] = corewar->arena[(process->pc + s % IDX_MOD) %
            MEM_SIZE];
